    ...
    public partial class FormProdukUtama : Form
    {
        ...
        private void LoadDataProduk()
        {
            ...
            using (SqlConnection conn = Koneksi.GetConnection())
            {
                ...
                {
                    conn.Open();
                    string query = @"SELECT p.Id, p.NamaProduk, p.Harga, p.Stok, p.Deskripsi,
                    k.NamaKategori
                    FROM Produk p
        ...
        {
            bool adaBarisDipilih = dgvProduk.SelectedRows.Count > 0;
            btnEdit.Enabled = adaBarisDipilih;
            btnHapus.Enabled = adaBarisDipilih;
        }
